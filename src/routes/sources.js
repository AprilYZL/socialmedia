...
import { startScrape, getScrapeStatus } from '../services/profileScraper.js';
import { getActiveAccount, isAccountId } from '../services/accounts.js';

export const sourcesRouter = Router();

// The repost targets a source post can be marked as uploaded to.
const CN_PLATFORMS = ['bilibili', 'xiaohongshu', 'douyin'];
// Short labels that fit on the per-post toggle buttons.
const CN_SHORT_LABELS = { bilibili: 'B站', xiaohongshu: '小红书', douyin: '抖音' };
...
sourcesRouter.get('/sources/:id', (req, res) => {
  ...
  const cnPlatforms = db
    .prepare(
      `SELECT id, display_name FROM platforms
       WHERE id IN (${CN_PLATFORMS.map(() => '?').join(',')})
       ORDER BY sort_order`
    )
    .all(...CN_PLATFORMS)
    .map((p) => ({ ...p, short: CN_SHORT_LABELS[p.id] || p.display_name }));
...
sourcesRouter.post('/sources/post/:postId/mark/:platformId', (req, res) => {
  ...
  if (!CN_PLATFORMS.includes(platformId)) return res.status(400).send('Not a repost target platform');
  ...
  if (existing) {
    ...
    db.prepare('INSERT INTO profile_post_marks (profile_post_id, platform_id) VALUES (?, ?)').run(
      post.id,
      platformId
    );
  }
  // Anchor keeps the grid scrolled to the card that was just toggled.
